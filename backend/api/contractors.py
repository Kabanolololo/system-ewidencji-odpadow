...
def list_contractors(filters: ContractorFilterParams = Depends(), db: Session = Depends(get_db)):
...
def create_contractor_online(contractor: ContractorOnlineCreate, db: Session = Depends(get_db)):
...
def create_contractor_offline(contractor: ContractorCreate, db: Session = Depends(get_db)):
...
def update_contractor(contractor_id: int, contractor: ContractorUpdate, db: Session = Depends(get_db)):
